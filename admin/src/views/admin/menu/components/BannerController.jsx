...
import AddBanner from "./AddBanner"; // Ensure you have this CSS file
...
function BannerController({menu}) {
    ...
    const { mutate } = useMutation(data => axiosPrivate.put(`/menu/${menu.menuId}`, {
        ...
        onSuccess: (data) => {
            ...
            toast("Banner başarıyla silindi");
        },
        onError: (error) => {
            toast.error("Banner silinirken bir hata oldu");
        },
    });
    ...
    return (
        <Card extra={"w-full h-full p-3 gap-6"}>
            <AddBanner isOpen={isAddBannerModalOpen} onClose={() => setIsAddBannerModalOpen(!isAddBannerModalOpen)}
                       banners={banners}/>
