...
const Products = () => {
    ...
    return (
        <div className="mt-3 grid h-fit grid-cols-2 lg:grid-cols-3 gap-5">
            <div className={"sticky col-span-2 lg:col-span-1 "}>
                <Card extra={"rounded-md border border-gray-200 bg-white p-6 shadow-lg max-h-dvh overflow-y-scroll"}>
                    <div className={"flex justify-between items-center"}>
                        <h2 className={"text-lg font-bold text-navy-700 dark:text-white"}>kategoriler</h2>
                        <Tooltip content={"yeni kategori ekle"} placement={"bottom"} trigger={<div
                            ...
                            {!addCategory ? <BsPlusSquare className={"w-5 h-5"}/> : <BsXSquare className={"w-5 h-5"}/>}
                        </div>}/>
                    </div>
                    {addCategory &&
                    ...
                    <div className={"flex flex-col justify-center items-center w-full "}>
                        {categories?.data?.map(category => (<div key={category.categoryId}
            ...
            <div className={"col-span-2 max-h-dvh overflow-y-scroll"}>
                ...
                {addProduct && <div className={"mb-3"}>
                    ...
                    <div className={"flex flex-row justify-center items-center w-full"}>
                        <hr
                            className="my-8 mx-6 h-0.5 border-t-0 bg-indigo-500 opacity-100 dark:opacity-50 w-full"
                        />
                        <span>
                            veya
                        </span>
